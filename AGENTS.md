...
- Package code lives in `src/codescape/util/`.
...
The test command includes the configured coverage gate. Keep coverage focused on `codescape.util`, and add or update tests with behavior changes.

## Change conventions

- Keep public APIs and the existing `src` layout stable unless the task requires otherwise.
- Use absolute imports from `codescape.util`.
